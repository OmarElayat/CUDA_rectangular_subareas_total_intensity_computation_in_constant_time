...
__global__ void MatrixMultiplykernel(float* A, float* B, float* C, int ARows, int ACols, int BRows, int BCols) {
    ...
    int Col = (blockIdx.x*2)*TILE_DIM_INT + threadIdx.x;

    int Col_2 = ((blockIdx.x*2)+1)*TILE_DIM_INT + threadIdx.x;

    int Col_3 = ((blockIdx.x*2)+2)*TILE_DIM_INT + threadIdx.x;

    int Col_4 = ((blockIdx.x*2)+3)*TILE_DIM_INT + threadIdx.x;

    __shared__ float As[TILE_DIM_INT][TILE_DIM_INT];
    __shared__ float Bs[TILE_DIM_INT][TILE_DIM_INT];
    __shared__ float Bs_2 [TILE_DIM_INT][TILE_DIM_INT];
	  __shared__ float Bs_3 [TILE_DIM_INT][TILE_DIM_INT];
	  __shared__ float Bs_4 [TILE_DIM_INT][TILE_DIM_INT];
	

    for (int k = 0; k < (TILE_DIM_INT + ACols - 1)/TILE_DIM_INT; k++) {
		
        //loading A to the shared memory once
        ...
        //loading B to the shared memory four times
        //first block
	    if (k*TILE_DIM_INT + threadIdx.y < BRows && Col < BCols){
        ...
        //second block
        if (k*TILE_DIM_INT + threadIdx.y < BRows && Col_2 < BCols){
            Bs_2[threadIdx.y][threadIdx.x] = B[(k*TILE_DIM_INT + threadIdx.y)*BCols + Col_2];
        }
        else{
            Bs_2[threadIdx.y][threadIdx.x] = 0.0;
        }
        //third block
        if (k*TILE_DIM_INT + threadIdx.y < BRows && Col_3 < BCols){
            Bs_3[threadIdx.y][threadIdx.x] = B[(k*TILE_DIM_INT + threadIdx.y)*BCols + Col_3];
        }
        else{
            Bs_3[threadIdx.y][threadIdx.x] = 0.0;
        }
        //fourth block
        if (k*TILE_DIM_INT + threadIdx.y < BRows && Col_4 < BCols){
            Bs_4[threadIdx.y][threadIdx.x] = B[(k*TILE_DIM_INT + threadIdx.y)*BCols + Col_4];
        }
        else{
            Bs_4[threadIdx.y][threadIdx.x] = 0.0;
        }
	    __syncthreads();

	    for (int n = 0; n < TILE_DIM_INT; ++n){
            CValue += As[threadIdx.y][n] * Bs[n][threadIdx.x];
            CValue_2 += As[threadIdx.y][n] * Bs_2[n][threadIdx.x];
            CValue_3 += As[threadIdx.y][n] * Bs_3[n][threadIdx.x];
            CValue_4 += As[threadIdx.y][n] * Bs_4[n][threadIdx.x];
    ...
    if(Row < CRows){
        if(Col_4 < CCols){
            C[(Row*CCols)+Col_4]=CValue_4;
        }
        if(Col_3 < CCols){
            C[(Row*CCols)+Col_3]=CValue_3;
        }
        if(Col_2 < CCols){
            C[(Row*CCols)+Col_2]=CValue_2;
        }
        if(Col < CCols){
...
void MatrixMultiplyOnDevice (float * M, float * N, float * P, int width, int heightN, int heightM , double & gflops)  {
    ...
    dim3 dimGrid (ceil((heightN/TILE_DIM)/4), ceil(heightM/TILE_DIM),1);
